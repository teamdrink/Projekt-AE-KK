	...
	public void calculatePath()
	{
		for (int i = 0; i < thePopulationData.size; ++i)
		{
			...
			for (int j = 0; j < theCityData.size - 1; ++j)
			{
				...
				{
					pathLenght[i] += theCityData.cityTab[thePopulationData.popTab[i][j]][thePopulationData.popTab[i][j
							+ 1]];
				} else
				{
					pathLenght[i] += theCityData.cityTab[thePopulationData.popTab[i][j
							+ 1]][thePopulationData.popTab[i][j]];
				}
			}
		}
	}
	...
	public void calculatePathSelection()
	{
		for (int i = 0; i < thePopulationData.size * 2; ++i)
		{
			...
			for (int j = 0; j < theCityData.size - 1; ++j)
			{
				...
				{
					pathLenghtAction[i] += theCityData.cityTab[thePopulationData.popTabAction[i][j]][thePopulationData.popTabAction[i][j
							+ 1]];
				} else
				{
					pathLenghtAction[i] += theCityData.cityTab[thePopulationData.popTabAction[i][j
							+ 1]][thePopulationData.popTabAction[i][j]];
				}
			}
		}
	}
